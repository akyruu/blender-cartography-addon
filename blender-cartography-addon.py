...
--------------------------------------------------------------------------
   ...
   3. Grant of Patent License. Subject to the terms and conditions of
      ...
      where such license configlies only to those patent claims licensable
   ...
   4. Redistribution. You may reproduce and distribute copies of the
      ...
      (d) If the Work includes a 'NOTICE' text file as part of its
          ...
          wherever such third-party notices normally configear. The contents
   ...
   7. Disclaimer of Warranty. Unless required by configlicable law or
      ...
      configropriateness of using or redistributing the Work and assume any
      risks associated with Your exercise of permissions under this License.

   8. Limitation of Liability. In no event and under no legal theory,
      whether in tort (including negligence), contract, or otherwise,
      unless required by configlicable law (such as deliberate and grossly
   ...
   APPENDIX: How to configly the Apache License to your work.

      To configly the Apache License to your work, attach the following
      boilerplate notice, with the fields enclosed by brackets '[]'
      replaced with your own identifying information. (Don't include
      the brackets!)  The text should be enclosed in the configropriate
      comment syntax for the file format. We also recommend that a
      file or class name and description of purpose be included on the
      same 'printed page' as the copyright notice for easier
      identification within third-party archives.

   Copyright 2020 Cyril Audibert (alias Akyruu)
   ...
   Unless required by configlicable law or agreed to in writing, software
...
class Logger:
    __level = LogLevel(config['logLevel'])
    ...
    def log(self, level: LogLevel, message: str):
        if level.value >= self.__level.value:
            ...
            print(now.strftime('%Y-%m-%d %H:%M:%S:%f'), # Timestamp
                    '[' + level.name + ']', # Level
                    self.name + ':', # Name
                    message) # Message
...
# Service - Reader ------------------------------------------------------------
class CartographyReaderMode(Enum):
    HEADER = 1
    ROOM = 2
    POINT = 3
    COMMENT = 4
...
class CartographyReader:
    ...
    def __init__(self, filepath):
        ...
        self.__switcher = {
            CartographyReaderMode.HEADER: self.__readHeader,
            CartographyReaderMode.ROOM: self.__readRoom,
            CartographyReaderMode.POINT: self.__readPoint,
            CartographyReaderMode.COMMENT: self.__readComment
        }
    ...
    def read(self):
        with open(self.filepath, 'r', encoding='utf8') as file:
            for line in file:
                ...
                line = line.strip()
                if not line:
                    self.__mode = CartographyReaderMode.COMMENT
                    continue
    ...
    def __readRoom(self, line: str):
        matches = self.__checkLine(line, 'room', ['((Room|Salle) [0-9]+) ([A-Za-z].+)'])
        m = matches[0]
        self.__logger.trace('Room line found #' + str(self.__row))
        
        # Get or create room
        name = m.group(1)
        room = self.__rooms.get(name, None);
        if room == None:
            self.__logger.debug('Create new room <' + name + '>')
        ...
        self.__column = 1
        self.__pointType = self.__checkPointType(m.group(3))
    ...
    def __readComment(self, line: str):
    ...
    def __checkLine(self, line: str, type: str, patterns: list, strict = True, columns = 0):
        ...
        if len(data) < count:
            if strict:
                self.__raiseError(line, type, self.__separator.join(patterns))
            return None
        ...
        for i in range(count):
            ...
            if m == None:
                if strict:
                    self.__raiseError(value, type, pattern)
                return None
            matches.append(m)
        
        return matches

    def __checkPointType(self, value: str, defaultValue = None):
        for pattern in self.__pointTypeMap:
    ...
    def __ignoreLine(self, line: str):
        self.__logger.info('Ignore <' + self.__formatValue(line) + '> (l.' + str(self.__row) + ')')

    def __raiseError(self, value: str, type: str, pattern: str):
        raise Exception('Invalid ' + type + ':' + ' <' + self.__formatValue(value) + '>' \
                    + ' (l.' + str(self.__row) + ', c.' + str(self.__column) + ').' \
                + ' Expected: <' + pattern + '>' \
                    + ' (case insensitive)')
...
class CartographyTemplate:
    ...
    __logger = Logger('CartographyTemplate')
    ...
    def __findObjectByName(self, name: str):
...
class CartographyCsvImportAction:
    __logger = Logger('CartographyCsvImportAction')
    ...
    def execute(self):
        # Read CSV file
        self.__logger.info('Read CSV file <' + self.filepath + '>')
        reader = CartographyReader(self.filepath)
        rooms = reader.read()
        self.__logger.info('CSV file <' + self.filepath + '> read with success!')

        # Read .blend template
        blendPath = r'D:\Dev\Blender\@Scripts\blender-cartography-addon\blender-cartography-template.blend' #FIXME use relative path
        self.__logger.info('Read .blend template <' + blendPath + '>')
        template = CartographyTemplate(blendPath)
        template.read()
...
class CartographyCsvImportOperator(Operator, ImportHelper):
...
def register():
    ...
    if config['production']:	# prod mode
        bpy.types.TOPBAR_MT_file_import.append(draw_menu)
    else:					# dev mode
        CartographyCsvImportAction(r'D:\Dev\Blender\@Scripts\blender-cartography-addon\samples\coordinates_01.tsv').execute()
